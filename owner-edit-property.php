<?php
/**
 * owner-edit-property.php — Edit Property Details
 */
...
$id = (int)($_GET['id'] ?? 0);
if ($id <= 0) { redirect('owner-dashboard.php'); }

// Fetch property with ownership check (admins can edit anything, owners only their own)
if (isAdmin()) {
    $stmt = $db->prepare("SELECT * FROM properties WHERE id = :id");
    $stmt->execute([':id' => $id]);
} else {
    $stmt = $db->prepare("SELECT * FROM properties WHERE id = :id AND owner_id = :oid");
    $stmt->execute([':id' => $id, ':oid' => $userId]);
}
$property = $stmt->fetch();

if (!$property) { redirect('owner-dashboard.php'); }

// Fetch all amenities
$allAmenities = $db->query("SELECT * FROM amenities ORDER BY name")->fetchAll();

// Fetch currently selected amenities
$amStmt = $db->prepare("SELECT amenity_id FROM property_amenities WHERE property_id = :id");
$amStmt->execute([':id' => $id]);
$currentAmenities = $amStmt->fetchAll(PDO::FETCH_COLUMN);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    elseif ($availRooms > $totalRooms)
        ...
        $status = isAdmin() ? $property['status'] : 'pending';

        $upd = $db->prepare("
            UPDATE properties
            SET name=:name, city=:city, address=:addr, price=:price, gender=:gender,
                description=:desc, image=:img, total_rooms=:tr, available_rooms=:ar,
                contact_phone=:cp, status=:status
            WHERE id=:id
        ");
        $upd->execute([
            ':name'=>$name, ':city'=>$city, ':addr'=>$address, ':price'=>$price,
            ':gender'=>$gender, ':desc'=>$description, ':img'=>$image,
            ':tr'=>$totalRooms, ':ar'=>$availRooms, ':cp'=>$contactPhone,
            ':status'=>$status, ':id'=>$id
        ]);

        // Sync amenities
        $db->prepare("DELETE FROM property_amenities WHERE property_id = :id")->execute([':id'=>$id]);
        if (!empty($selectedAm)) {
            $insAm = $db->prepare("INSERT INTO property_amenities (property_id,amenity_id) VALUES (?,?)");
            foreach ($selectedAm as $amId) {
                $insAm->execute([$id, (int)$amId]);
            }
        }
...
// Current values
$nameVal        = $_POST['name']          ?? $property['name'];
$cityVal        = $_POST['city']          ?? $property['city'];
$genderVal      = $_POST['gender']        ?? $property['gender'];
$addressVal     = $_POST['address']       ?? $property['address'];
$priceVal       = $_POST['price']         ?? $property['price'];
$totalRoomsVal  = $_POST['total_rooms']   ?? $property['total_rooms'];
$availRoomsVal  = $_POST['avail_rooms']   ?? $property['available_rooms'];
$contactPhoneVal= $_POST['contact_phone'] ?? $property['contact_phone'];
$descriptionVal = $_POST['description']   ?? $property['description'];
$imageVal       = $_POST['image']         ?? $property['image'];
$checkedAmenities = $_SERVER['REQUEST_METHOD'] === 'POST' ? ($selectedAm ?? []) : $currentAmenities;
?>
<!DOCTYPE html><html lang="en"><head>
<meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>Edit Property — PGFinder Owner</title>
  ...
  <div class="container" style="max-width:800px">
    <div class="owner-header">
      <div>
        ...
        <h1 class="section-title">Edit Property Details</h1>
    ...
    <form method="POST" class="owner-form">
        ...
        <div class="row g-3">
          <div class="col-12">
            ...
            <input type="text" class="form-input-custom" name="name"
                   placeholder="e.g. Sunrise Boys PG" required value="<?= e($nameVal) ?>">
          </div>
          <div class="col-md-6">
          ...
          <div class="col-md-6">
            ...
            <select class="form-input-custom" name="gender">
              <option value="male"   <?= ($genderVal==='male')?'selected':''?>>Boys Only</option>
              <option value="female" <?= ($genderVal==='female')?'selected':''?>>Girls Only</option>
              <option value="any"    <?= ($genderVal==='any')?'selected':''?>>Co-ed / Any</option>
            </select>
          </div>
          <div class="col-12">
            <label class="form-label-custom">Full Address *</label>
            <input type="text" class="form-input-custom" name="address"
                   placeholder="Street, Area, City - Pincode" required value="<?= e($addressVal) ?>">
          </div>
        </div>
      </div>
        ...
        <div class="row g-3">
          <div class="col-12">
            ...
            <textarea class="form-input-custom" name="description" rows="4"
                      placeholder="Describe your PG — facilities, location advantages, house rules…"><?= e($descriptionVal) ?></textarea>
          </div>
          <div class="col-12">
            ...
            <div id="img-preview" style="margin-top:10px; display: <?= empty($imageVal)?'none':'block' ?>">
              <img id="preview-img" src="<?= e($imageVal) ?>" alt="Preview"
        ...
        <div class="amenity-checkbox-grid">
          ...
          <label class="amenity-checkbox-item">
            <input type="checkbox" name="amenities[]" value="<?= $am['id'] ?>"
                   <?= in_array($am['id'], $checkedAmenities)?'checked':'' ?>>
      ...
      <div style="background:rgba(245,200,66,.08);border:1px solid rgba(245,200,66,.2);border-radius:10px;padding:1rem;font-size:.85rem;color:var(--text-secondary);margin-bottom:1.5rem">
        ...
        Editing will reset the property status to "Pending approval" until reviewed by our admin team.
      </div>
      <?php endif; ?>
